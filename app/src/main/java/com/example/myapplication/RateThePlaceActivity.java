       ...
       // System.out.println(data);
        ...
        ratingBar.setOnRatingBarChangeListener(new RatingBar.OnRatingBarChangeListener() {
            ...
            public void onRatingChanged(RatingBar ratingBar, float rating, boolean fromUser) {
                ...
                if (val>0 && val <=1) ratingScale.setText("Okropne :C");
                else if (val>1 && val <=2) ratingScale.setText("Nienajgorsze :(");
                else if (val>2 && val <=3) ratingScale.setText("Neutralne :|");
                else if (val>3 && val <=4) ratingScale.setText("Dobre :)");
                else if (val>4 && val <=5) ratingScale.setText("Super! :D");
        ...
        sendCommentButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (commentMessage.getText().toString().equals("")) {
                    Toast.makeText(getApplicationContext(), "Proszę wpisz treść komentarza", Toast.LENGTH_SHORT).show();
                }
                else {
                    Toast.makeText(getApplicationContext(), "Dziękujemy za Twoją opinie", Toast.LENGTH_SHORT).show();
                    Rate rate = new Rate(commentMessage.getText().toString(),ratingBar.getRating(),currentTime, false, currentUser);
                    ref.setValue(rate);
                    finish();
                 //   Intent intent = new Intent(RateThePlaceActivity.this, ItemDetailsActivity.class);
                   // startActivity(intent);
                }
            }
        });
